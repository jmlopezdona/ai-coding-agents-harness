...
El primer supuesto que se cae es **"todo PR pasa por un humano"**. No por ideología, sino por aritmética. Si tienes 7 ingenieros y el agente abre 20 PRs al día por persona, son 140 PRs diarios. Asumiendo que un humano pueda hacer una review decente en 15 minutos, son 35 horas-humano de revisión por día. No salen las cuentas.
...
6. Vuelve al paso 3 hasta que todos los reviewers están satisfechos.
...
**2. Rutina — revisado por otros agentes.** La mayoría del trabajo. Implementación de una feature pequeña, fix de un bug bien delimitado, tests adicionales. Un agente revisor lee, comenta, el agente original itera. Si convergen, mergean. El humano podría mirar pero no tiene por qué.

**3. Sensible — humano en el loop.** Cambios en módulos marcados como críticos (auth, billing, migrations, contratos públicos). El humano revisa, decide, posiblemente delega de vuelta al agente para iterar. La marca de "sensible" no la decide cada PR; la decide la arquitectura, codificada en metadatos del repo.
...
Esto suena irresponsable. En un equipo de bajo throughput lo sería. En un equipo donde el agente puede arreglar lo que rompa en minutos, **el coste de esperar es mayor que el coste de romper**. La disciplina se reubica (otra vez) hacia detección rápida y rollback barato. Si esos dos no están, no apliques este patrón. Si están, no aplicarlo es dejar dinero en la mesa.
...
- **Rollback trivial.** Capacidad de revertir un PR sin ceremonia, idealmente automática si una alerta se dispara dentro de N minutos del merge.
...
Una propiedad importante: **las reviews agente-a-agente deben tener foco**. Un único agente intentando revisar "todo" produce reviews mediocres. Múltiples agentes revisores con prompts especializados — uno para arquitectura, otro para seguridad, otro para tests, otro para coherencia con docs activas — producen señal mucho mejor. Stripe lo formaliza con su modelo de "blueprints" combinando nodos determinísticos y agénticos en pipelines explícitos.

## El humano como reviewer de excepción
...
- **Aplicar criterio que no se puede codificar.** Decisiones de producto, trade-offs estratégicos, juicio sobre si una solución "encaja" con la dirección del proyecto.
- **Identificar patrones que merecen subir al harness.** Si está corrigiendo lo mismo dos veces, eso no es review, es señal de que falta un guide o un sensor. Su trabajo más valioso es traducir esa observación en una mejora del harness.
